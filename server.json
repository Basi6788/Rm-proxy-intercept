...
// In-memory array logs store karne ke liye (Vercel serverless me yeh temporary hota hai)
let requestLogs = []; 
...
app.get('/', (req, res) => {
    ...
    <body class="bg-[#0a0a0a] text-gray-300 font-mono min-h-screen p-4 sm:p-8">
        <div class="max-w-6xl mx-auto">
            <header class="flex justify-between items-center border-b border-green-900/50 pb-4 mb-6">
                ...
                <div class="flex items-center gap-2 px-3 py-1 bg-green-900/20 border border-green-500/30 rounded-full">
                    ...
                    <span class="text-xs text-green-400 font-semibold tracking-wide">LIVE SYSTEM</span>
                </div>
            </header>
        ...
        <script>
                ...
                try {
                    ...
                    if (logs.length !== lastLogCount) {
                        ...
                        logs.forEach(log => {
                            ...
                            const encodedData = rawData.replace(/'/g, "\\'").replace(/"/g, '&quot;');
                            ...
                            <div class="bg-[#111] border border-gray-800 rounded-lg overflow-hidden log-enter transition-all hover:border-green-900/50">
                                <div class="flex justify-between items-center bg-[#1a1a1a] px-4 py-2 border-b border-gray-800">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <button onclick="copyToClipboard(this, '\${encodedData}')" class="flex items-center text-xs text-green-500 border border-green-500/50 hover:bg-green-500/10 px-2 py-1 rounded transition-colors">
                                ...
                                <div class="p-4 grid grid-cols-1 lg:grid-cols-2 gap-4 text-xs">
                                    <div class="bg-[#0a0a0a] p-3 rounded border border-gray-800/50">
                                        ...
                                        <pre class="overflow-x-auto text-gray-400">\${JSON.stringify(log.requestBody, null, 2) || '{}'}</pre>
                                    </div>
                                    <div class="bg-[#0a0a0a] p-3 rounded border border-gray-800/50">
                                        <div class="text-green-500 mb-2 font-bold opacity-70 border-b border-gray-800 pb-1">SERVER RESPONSE</div>
                                        <pre class="overflow-x-auto text-gray-400">\${JSON.stringify(log.responseBody, null, 2) || '{}'}</pre>
                                    </div>
                                </div>
                            </div>
                            \`;
            ...
            // Har 1.5 second me naye logs fetch karega
...
app.all('*', async (req, res) => {
    ...
    try {
        // Headers forward karne ki tyari (Host change karna zaroori hai Vercel ke bahar)
        const forwardHeaders = { ...req.headers };
        delete forwardHeaders.host; 
        delete forwardHeaders['content-length']; // Fetch khud handle karega
        ...
        // Response ko text me nikalna takay JSON or HTML dono capture ho sakein
        const responseText = await response.text();
        
        let parsedResponse = responseText;
        try { parsedResponse = JSON.parse(responseText); } catch(e) {} // Agar JSON hai to parse karo
        ...
        // Response wapis app ko bhejna (Almost 0ms overhead)
...
// Vercel serverless environment ke liye app ko export karna zaroori hai
module.exports = app;

// Agar local test karna chaho toh isay uncomment kardo:
